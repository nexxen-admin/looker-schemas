...
#
  dimension: house_impressions {
    type: number
    hidden: yes
    sql: case when ${campaign_details_base.isfiller} then ${impressions} else 0 end ;;
  }
  ...
  dimension: non_house_impressions {
    type: number
    hidden: yes
    sql: case when not ${campaign_details_base.isfiller} then ${impressions} else 0 end ;;
  }
  ...
  measure: sum_good_requests {
    ...
    value_format_name: decimal_0
    sql: CASE WHEN ${placement_details_base.fallback_type_id} = 1 then (${publisher_requests} - CASE WHEN ${TABLE}.ADSERVER_CODE = 1 then ${publisher_requests} else 0 end) ELSE '0' End + CASE WHEN ${TABLE}.ADSERVER_CODE = -5 then ${publisher_requests} ELSE '0' End + CASE WHEN ${TABLE}.ADSERVER_CODE = 0 then ${publisher_requests} ELSE '0' End + CASE WHEN ${TABLE}.ADSERVER_CODE = -3 then ${publisher_requests} ELSE '0' End +CASE WHEN ${TABLE}.ADSERVER_CODE = 1 then ${publisher_requests} else 0 end ;;
  }
  ...
  measure: sum_network_redirect_requestss {
    ...
    value_format_name: decimal_0
    sql: CASE WHEN ${placement_details_base.fallback_type_id} = 1 then (${publisher_requests} - CASE WHEN ${TABLE}.ADSERVER_CODE = 1 then ${publisher_requests} else 0 end) else '0' END;;
  }
...
# =Served Requests?
  ...
  measure: dst_first_delivery_date {
    ...
    sql: case when ${platform_client.use_daylight_saving} then MIN(case when ${billable_units} > 0 then ${local_demand_date_raw} end)
  ...
  measure: sum_demand_gross_spend_usd {
    ...
    value_format_name: decimal_2
    sql:( ${demand_revenue} * ${demand_usd_exchange_rate}) * (1+(${flight_media_details_base.agency_fee}))  ;;
    drill_fields: [platform_client_id, campaign, flight, flight_media, sum_demand_revenue]
  }
  ...
  dimension: hour_minute_of_day {
    type: number
    hidden: yes
    sql: ${hour_of_day} * 100 + coalesce(${daypart.minute_of_hour}, ${minute_of_hour}, 0) ;;
  }

  dimension: hour_minute_of_day_derived {
    type: number
    hidden: yes
    sql: ${hour_of_day_derived} * 100 + coalesce(${daypart.minute_of_hour}, ${minute_of_hour}, 0) ;;
  }
  ...
  dimension: hour_description {
    ...
    This is based on the day and time of the viewer."
    sql: coalesce(${daypart.hour_description}, ${TABLE}.HOUR_DESCRIPTION) ;;
...
# ID >> needs view
  ...
  measure: sum_publisher_requests {
    ...
    drill_fields: [placement_details_base.platform_client, placement_details_base.partner_name, placement_id, sum_publisher_requests]
  }
  ...
  measure: avg_winning_bid {
    ...
    value_format_name: decimal_2
    sql: ${winning_bid} ;;
    drill_fields: [placement_details_base.platform_client, placement_details_base.partner_name, placement_id, avg_winning_bid]
  }
  ...
  measure: sum_winning_bids {
    ...
    value_format_name: decimal_0
    sql: ${winning_bids} ;;
    drill_fields: [placement_details_base.platform_client, placement_details_base.partner_name, placement_id, sum_winning_bids]
  }
  ...
  measure: sum_impressions_last_hour {
    ...
    value_format_name: decimal_0
    sql: case when ${gmt_date_raw} = ${load_tracking.load_through_date} then ${impressions} else 0 end ;;
  }
  ...
  measure: total_cost_without_tech_fee {
    ...
    sql: CONCAT(CASE WHEN count(distinct ${plan_currency.currency_id}) = 1
          THEN MAX(${plan_currency.currency_symbol})
