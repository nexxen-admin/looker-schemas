view: v_reach_and_frequency {
  derived_table: {
    sql: SELECT
            ...
            {% if v_campaign_details_base._in_query %} JOIN DIM.CAMPAIGN_DETAILS_BASE_VIEW CD on FMD.campaign_id = CD.campaign_id {% endif %}
            {% if v_advertiser_brand_details._in_query %} JOIN DIM.ADVERTISER_BRAND_DETAILS_VIEW ABD on CD.advertiser_brand_id = ABD.advertiser_brand_id {% endif %}
            WHERE {% condition v_daily_core_stats.demand_date %} DCS.DEMAND_DATE {% endcondition %} AND
              {% condition v_campaign_details_base.campaign_id %} FMD.CAMPAIGN_ID {% endcondition %} AND
              {% condition v_campaign_details_base.campaign_name %} CD.CAMPAIGN_NAME {% endcondition %} AND
              {% condition v_campaign_details_base.pl_plan_id %} CD.PL_PLAN_ID {% endcondition %} AND
              {% condition v_campaign_details_base.plan_name %} CD.PL_PLAN_NAME {% endcondition %} AND
              {% condition v_advertiser_brand_details.advertiser_name %} ABD.ADVERTISER_NAME {% endcondition %} AND
        ...
        {% if v_campaign_details_base._in_query %} JOIN DIM.CAMPAIGN_DETAILS_BASE_VIEW CD on FMD.campaign_id = CD.campaign_id {% endif %}
        {% if v_advertiser_brand_details._in_query %} JOIN DIM.ADVERTISER_BRAND_DETAILS_VIEW ABD on CD.advertiser_brand_id = ABD.advertiser_brand_id {% endif %}
        WHERE {% condition v_daily_core_stats.demand_date %} CUS.DEMAND_DATE {% endcondition %} AND
          {% condition v_campaign_details_base.campaign_id %} FMD.CAMPAIGN_ID {% endcondition %} AND
          {% condition v_campaign_details_base.campaign_name %} CD.CAMPAIGN_NAME {% endcondition %} AND
          {% condition v_campaign_details_base.pl_plan_id %} CD.PL_PLAN_ID {% endcondition %} AND
          {% condition v_campaign_details_base.plan_name %} CD.PL_PLAN_NAME {% endcondition %} AND
  ...
  measure: reach {
    ...
    Year)."
    value_format_name: decimal_0
    sql: COALESCE(${v_daily_core_stats.sum_impressions} / NULLIF(${average_frequency},0), ${v_daily_core_stats.sum_impressions})
      ;;
  }
