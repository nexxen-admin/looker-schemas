view: sno_q_viewer_equi_imps {
  derived_table: {
    sql:
        ...
        FROM(
        SELECT
        {% if sno_q_viewer_equi_imps.age._in_query %} v.age, {% endif %}
      ...
      JOIN VIEWERSHIP.MEDIA_CONTENT mc
       ...
       WHERE rta.timeperiod_type = 'q'
           ...
           AND {% condition filter_timeperiod_quarter_key %} vf.timeperiod_quarter_key  {% endcondition %}
           AND {% condition campaign_id %} cc.campaign_id  {% endcondition %}
       GROUP BY {% if sno_q_viewer_equi_imps.age._in_query %}  v.age, {% endif %}
                {% if sno_q_viewer_equi_imps.network._in_query %} mc.media_content_parent, {% endif %}
                {% if sno_q_viewer_equi_imps.daypart._in_query %} mc.media_daypart_id , {% endif %}
                {% if sno_q_viewer_equi_imps.program._in_query %} mc.media_content_name , {% endif %}
                {% if sno_q_viewer_equi_imps.event_date._in_query %} vf.event_date , {% endif %}
          ...
          {% elsif sno_q_viewer_equi_imps.network._in_query
            ...
            and sno_q_viewer_equi_imps.program._in_query %} media_content_parent, media_daypart_id, media_content_name, event_date
          {% else %} TRUE
          {% endif %}
  ...
  filter: filter_timeperiod_quarter_key {
    view_label: "Viewing Metrics"
    description: "Select the quarter to get ad reach/frequency report. Eg. Q2 2017 is 201702"
    label: "Quarter"
    type: number
  }
  ...
  dimension: primary_key {
    ...
    measure: imps_per_unit {
      ...
      sql: ${equi_ad_imps}/NULLIF(${sno_q_ad_units.total_units},0);;
    }
