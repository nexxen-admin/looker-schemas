view: v_daily_data_usage {
  ...
  measure: adjusted_cost {
    ...
    value_format_name: decimal_2
    sql: ${data_cost} - (case
            when ${v_placement_details_base.device_type} = 'Connected TV' then 0
            when ${v_placement_details_base.device_type} in ('PC','Mobile') and (lower(${v_retargeting_attribute.description}) like lower('%Comscore%') or lower(${v_retargeting_attribute.description}) like lower('%Oracle%')) then 0
            when $v_{placement_details_base.device_type} in ('PC','Mobile') and (lower(${v_retargeting_attribute.description}) like lower('%Webbula > B2B%') or lower(${v_retargeting_attribute.description}) like lower('%Kantar - Shopcom%')) then 0.1 * ${data_cost}
  ...
  measure: sum_impressions {
    ...
    sql: ${impressions} ;;
    drill_fields: [v_source_provider.description,v_retargeting_attribute.behavioral_category,v_retargeting_attribute.description,sum_impressions]
  }
  ...
  measure: billable_usage {
    ...
    drill_fields: [v_source_provider.description,v_retargeting_attribute.behavioral_category,v_retargeting_attribute.description,billable_usage]
  }
