view: v_placement_frequency_cap_violations {
      ...
      SELECT
        ...
        FROM
          (
            ...
            LEFT JOIN dim.frequency_cap_view fc ON fc.level = 1 AND fc.ref_id = r1.placement_id AND r1.eventtime >= fc.start_time and r1.eventtime < coalesce(fc.end_time, TO_TIMESTAMP('9999-12-12', 'YYYY-MM-DD'))
            LEFT JOIN rawdb.raw_impression r2 ON ({% condition v_raw_impression.demand_date_date %} r2.demand_date {% endcondition %})
  ...
  dimension: max_exposures {
    label: "Placement Level Frequency Cap"
  ...
  dimension: exposure_minutes{
    label: "Placement Level Frequency Cap Period (in minutes)"
    view_label: "Placement"
    type: number
  ...
  measure: sum_of_violations {
    type: sum
    label: "Placement Level Frequency Cap Violations"
    view_label: "Placement"
    sql: ${is_violation} ;;
  }

  measure: pct_of_violations {
    label: "% of Placement Level Frequency Cap Violations"
    type: number
