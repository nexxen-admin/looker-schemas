view: just_bare_chicken_5_27_22 {
  derived_table: {
    sql: SELECT  date::date as "Date",
        ...
        CASE WHEN flight_id IN(4320336, 4408076, 4408086, 4408096, 4454156)  THEN 'CTV + BT + Zip Targeting'
             WHEN flight_id IN (4408246,4408256, 4455176) THEN 'CTV 1% Added Value'
             WHEN flight_id IN (4408216, 4408226, 4408236, 4455096) THEN 'All Screen 1% AV'
        ...
        CASE WHEN flight_id IN(4320336, 4320346, 4408076, 4408146, 4408086, 4408166, 4408096, 4408186, 4454156, 4455066) THEN (SUM(impressions)/1000) * 21.75
          ELSE 0 END AS "Spend"
...
WHERE date >= CURRENT_DATE()-7
  ...
  and flight_id IN (4320336, 4320346, 4408076, 4408146, 4408246, 4408216, 4408086, 4408166, 4408256, 4408226, 4408096,4408186,4408266,4408236,4454156, 4455066, 4455176, 4455096)
