view: daily_core_stats {
  ...
  measure: grp_audience_size {
    ...
    sql: ${flight_media_details_base.grp_audience_size} ;;
  }
  ...
  measure: trp_audience_size {
    ...
    sql: ${flight_media_details_base.trp_audience_size} ;;
  }
  ...
  measure: sum_billable_units_yesterday_gmt {
    ...
    sql: case when ${gmt_raw} = case when ${demand_mart_load_tracking_start_timezone.utc_offset} > 0 then dateadd(d, -1, ${demand_mart_load_tracking.load_through_date}) else ${demand_mart_load_tracking.load_through_date} end then ${billable_units} else 0 end ;;
  }
  ...
  measure: billable_units_pct {
    ...
    sql: ${sum_billable_units} / nullif(${flight_demand_units_budget.sum_units}, 0) ;;
  }
  ...
  measure: gross_spend_usd {
    ...
    value_format_name: decimal_2
    sql: (${demand_revenue} * (1+(${flight_media_details_base.agency_fee}))) * ${usd_exchange_rate};;
  }
  ...
  dimension: delivery_units {
    type: number
    hidden:  yes
    sql:case when ${campaign_details_base.cpu_type_id} = 5
    then  ${TABLE}.DEMAND_REVENUE * (1+(${flight_media_details_base.agency_fee})) else  ${TABLE}.UNITS end   ;;
  }
  ...
  measure: pacing {
    ...
    hidden: yes
    sql: case when ${campaign_details_base.max_cpu_type_id} in (4, 5) then coalesce(${gross_spend}::float / nullif(${expected_delivery.expected_gross_spend}::float, 0), 0)
              else coalesce(${sum_billable_units}::float / nullif(${expected_delivery.expected_units}::float, 0), 0) end ;;
  }
  ...
  measure: average_units_by_day {
    ...
    sql: CASE WHEN ${campaign_days.campaign_remaining_days} > 0 and ${campaign_details_base.max_contracted_units} > ${sum_billable_units}
          THEN (${campaign_details_base.max_contracted_units} - ${sum_billable_units}) / ${campaign_days.campaign_remaining_days}
          ELSE 0 END ;;
  }
  ...
  measure: average_spend_by_day {
    ...
    hidden: yes
    description: "Planned Units minus Served Billable Units divided by Remaining Days"
    sql: CASE WHEN ${campaign_days.campaign_remaining_days} > 0 and ${campaign_details_base.max_planned_spend} > ${sum_demand_revenue}
          THEN (${campaign_details_base.max_planned_spend} - ${sum_demand_revenue}) / ${campaign_days.campaign_remaining_days}
          ELSE 0
          END ;;
  }
  ...
  measure: daily_goal {
    ...
    value_format_name: decimal_0
    sql:  CASE WHEN ${campaign_details_base.max_cpu_type_id} IN (4, 5)
  ...
  measure: house_served {
    ...
    sql: ${impressions} ;;
    filters: {
      field: campaign_details_base.isfiller
      value: "yes"
      }
    }
  ...
  measure: remaining_units {
    ...
    value_format_name: decimal_0
    sql: CASE WHEN COALESCE(1.0 * ${campaign_demand_units_budget.sum_campaign_delivery_units} - ${sum_billable_units},0) >= 0 THEN COALESCE(1.0 * ${campaign_demand_units_budget.sum_campaign_delivery_units} - ${sum_billable_units},0) ELSE 0 END  ;;
  }
  ...
  measure: target_units {
    ...
    value_format_name: decimal_0
    sql: (${campaign_days.campaign_total_days} / NULLIF(${campaign_days.campaign_elapsed_days},0)) * ${sum_billable_units};;
  }
  ...
  measure: percent_target {
    ...
    value_format_name: percent_2
    sql: ${target_units}/NULLIF(${campaign_demand_units_budget.sum_campaign_delivery_units},0);;
  }
  ...
  measure: shortfall_units {
    ...
    value_format_name: decimal_0
    sql: CASE WHEN (${campaign_demand_units_budget.sum_campaign_delivery_units} - (COALESCE(${sum_billable_units}*${campaign_days.campaign_remaining_days}/NULLIF(${campaign_days.campaign_elapsed_days},0),0) + ${sum_billable_units})) >= 0 THEN (${campaign_demand_units_budget.sum_campaign_delivery_units} - (COALESCE(${sum_billable_units}*${campaign_days.campaign_remaining_days}/NULLIF(${campaign_days.campaign_elapsed_days},0),0) + ${sum_billable_units})) ELSE 0 END ;;
  }
  ...
  measure: shortfall_revenue {
    ...
    value_format_name: decimal_2
    sql: CASE WHEN (${campaign_demand_units_budget.campaign_target_gross_spend} - (COALESCE(${gross_spend}*${campaign_days.campaign_remaining_days}/NULLIF(${campaign_days.campaign_elapsed_days},0),0) + ${gross_spend})) >= 0 THEN (${campaign_demand_units_budget.campaign_target_gross_spend} - (COALESCE(${gross_spend}*${campaign_days.campaign_remaining_days}/NULLIF(${campaign_days.campaign_elapsed_days},0),0) + ${gross_spend})) ELSE 0.00 END ;;
  }
  ...
  measure: target_pacing {
    ...
    value_format_name: percent_2
    sql: COALESCE(1.0*${campaign_days.campaign_elapsed_days}/NULLIF(${campaign_days.campaign_total_days},0),0) ;;
  }
  ...
  measure: forecast_delivery {
    ...
    value_format_name: percent_2
    sql: COALESCE(1.0*(${campaign_demand_units_budget.sum_campaign_delivery_units}-${shortfall_units})/NULLIF(${campaign_demand_units_budget.sum_campaign_delivery_units},0),0) ;;
  }
  ...
  measure: percent_completed {
    ...
    value_format_name: percent_2
    sql: COALESCE(${sum_units} / NULLIF(${campaign_details_base.sum_contracted_units}, 0), 0) ;;
  }
  ...
  measure: demo_index {
    ...
    value_format_name: decimal_0
    sql: COALESCE(${sum_effective_units} / ${sum_billable_impressions} / NULLIF(${flight_media_details_base.max_democomp}, 0) * 100, 0) ;;
  }
  ...
  measure: revenue_booked {
    ...
    value_format_name: decimal_0
    sql: ((CASE WHEN ${campaign_details_base.max_cpu_type_id} = 1 THEN 0.001 ELSE 1.00 END)
          * ${flight_details.max_cpu}
          * (CASE WHEN ${campaign_details_base.is_filler} = 1 THEN 0
          ELSE ${flight_details.sum_units} END));;
  }
