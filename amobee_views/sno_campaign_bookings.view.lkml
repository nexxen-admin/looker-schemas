 ...
 dimension: reconciled_revenue  {
  ...
  measure: ccp {
    ...
    sql: CASE WHEN (${ccp_days}) != 0
               THEN (${insertion_order_demand_units_budget.sum_budget}- ${sum_reconciled_revenue}) /(${ccp_days})
               ELSE 0 END;;
  }
  ...
  measure: daily_revenue_run_rate {
    ...
    type:  number
    value_format_name: decimal_2
    sql: coalesce(${daily_core_stats.monthly_revenue_delivered} / nullif(${io_current_month_days_delivered}, 0), 0) ;;
  }
  ...
  measure: io_campaign_status_finished_paused_ended {
    ...
    sql: MIN(${TABLE}.io_campaign_status_finished_paused_ended) AND ${daily_core_stats.monthly_revenue_delivered} = 0 AND NOT MIN(${TABLE}.io_format_type) ;;
  }
