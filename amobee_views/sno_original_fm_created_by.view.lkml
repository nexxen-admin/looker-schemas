...
(
  ...
  from
    dim.flight_media_details_base as fmd
  where
    {% condition flight_media_details_base.begin_datetm %} fmd.begin_datetm {% endcondition %} and
    {% condition flight_media_details_base.end_datetm %} fmd.end_datetm {% endcondition %} and
    fmd.flight_media_active
) as t
join dim.user_profile up on up.USERID = t.createdby
where
  t.rn = 1
      ;;
  }
