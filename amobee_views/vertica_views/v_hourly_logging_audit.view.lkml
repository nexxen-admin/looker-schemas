...
FROM (
  ...
  WHERE (
      ...
      AND {% condition v_hourly_logging_audit.keydate %} hr.keydate {% endcondition %}
      )
  GROUP BY hr.KEYDATE
         ,fmd.FLIGHT_MEDIA_ID
  ) t1
LEFT OUTER JOIN (
  ...
  WHERE (
      {% condition v_flight_media_details_base.flight_media_id %} ha.flight_media_id {% endcondition %}
      AND ha.PLACEMENT_ID = 1221
      AND {% condition v_hourly_logging_audit.keydate %} ha.GMT_DATE {% endcondition %}
      )
  GROUP BY ha.FLIGHT_MEDIA_ID
          ,ha.GMT_DATE
  ) t2 ON (t1.FLIGHT_MEDIA_ID = t2.FLIGHT_MEDIA_ID AND t1.KEYDATE = t2.KEYDATE)
JOIN DIM.FLIGHT_MEDIA_DETAILS_BASE_VIEW fmd2 ON (t1.FLIGHT_MEDIA_ID = fmd2.FLIGHT_MEDIA_ID)
WHERE (
    {% condition v_flight_media_details_base.flight_media_id %} t1.flight_media_id {% endcondition %}
      AND {% condition v_hourly_logging_audit.keydate %} t1.KEYDATE {% endcondition %}
    )
