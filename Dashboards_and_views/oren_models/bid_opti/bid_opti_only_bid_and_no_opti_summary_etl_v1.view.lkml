view: bid_opti_only_bid_and_no_opti_summary_etl_v1 {
    derived_table: {
      ...
      data_totals as (
        ...
        from raw_data_4_models
        group by 1
        HAVING count(distinct case when requests>0 and opti in ('bidfloor','no_opti') then opti else null end) = 2
               AND count(distinct case when opti not in ('bidfloor','no_opti') then opti else null end) = 0
        ),
        ...
        (sum(margin) - sum(case when opti='no_opti' then scaled_margin else 0 end))/(nullif(sum(margin),0)) as scaled_margin_ratio_to_no_opti,
        sum(margin) - sum(case when opti='no_opti' then scaled_margin else 0 end) as scaled_margin_diff_to_no_opti,
        (sum(supply_margin) - sum(case when opti='no_opti' then scaled_supply_margin else 0 end))/(nullif(sum(supply_margin),0)) as scaled_supply_margin_ratio_to_no_opti,
