view: v_daily_data_usage_not_active_30 {
  derived_table: {
    sql: WITH active AS (
SELECT
...
WHERE ((( v_daily_data_usage.DEMAND_DATE  ) >= (((DATE_TRUNC('day', CURRENT_TIMESTAMP AT TIME ZONE 'UTC') + (-29 || ' day')::INTERVAL))) AND ( v_daily_data_usage.DEMAND_DATE  ) < ((((DATE_TRUNC('day', CURRENT_TIMESTAMP AT TIME ZONE 'UTC') + (-29 || ' day')::INTERVAL) + (30 || ' day')::INTERVAL))))) AND ((COALESCE(v_daily_data_usage.DEMAND_DATE, '9999-12-31') >= (SELECT MIN(DEMAND_DATE) FROM RAWDB.daily_data_usage)))
GROUP BY 1, 2, 3
),
all_at AS (
SELECT
