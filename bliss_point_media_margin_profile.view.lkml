view: bliss_point_media_margin_profile {
  derived_table: {
    sql: /* Partner_Audience_RevShare */
      ...
      Exchange_Data as (
      Select date_trunc('month',ad.event_time)::date as event_month,
      c.agency_name,
      c.agency_id,
      case when c.agency_id = '439896' then 'Self-Service'
      else 'Managed-Service' end as buy_type,
      ad.campaign_id,
      coalesce(ddd.deal_id,'OMP') as DSP_Deal_ID,
      sum(ad.impression_pixel) as Impressions,
      sum(ad.revenue) as Revenue,
      sum(ad.cogs) as Cost,
      sum((ssp.platform_fee_spend / 100) * ad.cogs) as SSP_Platform_Fee,
      sum(pub_platform_fee) as Pub_Platform_Fee,
      sum(ad.revenue * b.Rebate_Percent) * -1 as Barter_Rebate
      From andromeda.ad_data_daily ad
      inner join dwh.campaign c on c.flight_id::varchar = ad.campaign_id::VARCHAR
      and c.agency_id in ('439896','446496')
      Left outer join dsp_deal_data ddd on ddd.flight_id::varchar = ad.campaign_id::varchar
      and ddd.event_month = date_trunc('month',ad.event_time)
      and ddd.deal_id = ad.rx_deal_id
      ...
      Where ad.event_time >= '2023-03-23'
      and ad.event_time < current_date()
      and ad.rx_dsp_name ilike 'tremor%'
      and ad.impression_pixel > 0
      Group by 1, 2, 3, 4, 5, 6
      ),

      Current_results as (
      Select coalesce(dsp.event_month,ex.event_month) as event_month,
      coalesce(dsp.agency_name,ex.agency_name) as agency_name,
      coalesce(dsp.agency_id,ex.agency_id) as agency_id,
      coalesce(dsp.buy_type,ex.buy_type) as buy_type,
      coalesce(dsp.flight_id::varchar,ex.campaign_id::varchar) as flight_id,
      coalesce(dsp.Deal_id,ex.DSP_Deal_id) as Deal_id,
      dsp.impressions as DSP_Impressions,
      --coalesce(ex.impressions,0) as Exch_Impressions,
      dsp.revenue as DSP_Revenue,
      --dsp.Inv_Cost as DSP_Inv_Cost,
      dsp.Data_Cost as DSP_Data_Cost,
      dsp.Partner_Cost as DSP_Partner_Cost,
      dsp.Platformfee_Profit as DSP_Platformfee_Profit,
      dsp.Platform_Cost as DSP_Platform_Cost,
      --coalesce(ex.Revenue,0) as Exch_Revenue,
      coalesce(ex.Cost,0) as Exch_Cost,
      coalesce(ex.SSP_Platform_Fee,0) as SSP_Platform_Fee,
      coalesce(ex.Pub_Platform_Fee,0) as Pub_Platform_Fee,
      coalesce(ex.Barter_Rebate,0) as Barter_Rebate
      From DSP_Data dsp
      full join Exchange_Data ex on ex.event_month = dsp.event_month
      and ex.agency_id = dsp.agency_id
      and ex.campaign_id::varchar = dsp.flight_id::varchar
      and ex.dsp_deal_id = dsp.deal_id
      ),
      ...
      sum(coalesce(l.dsp_impressions,0)) + sum(coalesce(c.dsp_impressions,0)) as DSP_Impressions,
      sum(coalesce(l.DSP_Revenue,0)) + sum(coalesce(c.DSP_Revenue,0)) as DSP_Revenue,
      sum(coalesce(l.DSP_Data_Cost,0)) + sum(coalesce(c.DSP_Data_Cost,0)) as DSP_Data_Cost,
      sum(coalesce(l.DSP_Partner_Cost,0)) + sum(coalesce(c.DSP_Partner_Cost,0)) as DSP_Partner_Cost,
      sum(coalesce(l.DSP_Platformfee_Profit,0)) + sum(coalesce(c.DSP_Platformfee_Profit,0)) as DSP_Platformfee_Profit,
      sum(coalesce(l.DSP_Platform_Cost,0)) + sum(coalesce(c.DSP_Platform_Cost,0)) as DSP_Platform_Cost,
      sum(coalesce(l.Exch_Cost,0)) + sum(coalesce(c.Exch_Cost,0)) as Exch_Cost,
      sum(coalesce(l.SSP_Platform_Fee,0)) + sum(coalesce(c.SSP_Platform_Fee,0)) as SSP_Platform_Fee,
      sum(coalesce(l.Pub_Platform_Fee,0)) + sum(coalesce(c.Pub_Platform_Fee,0)) as Pub_Platform_Fee,
      sum(coalesce(l.Barter_Rebate,0)) + sum(coalesce(c.Barter_Rebate,0)) as Barter_Rebate
      From Current_results c
      full join bi.SVC_BP_E2E_Base_Metrics_Up_To_3_23_23 L on l.event_month = c.event_month
      ...
      sum(fa.Exch_Cost) as Exch_Cost,
      sum(fa.SSP_Platform_Fee) as SSP_Platform_Fee,
      sum(fa.Pub_Platform_Fee) as Pub_Platform_Fee,
      sum(fa.Barter_Rebate) as Barter_Rebate,
      ...
      0 as Exch_Cost,
      0 as SSP_Platform_Fee,
      0 as Pub_Platform_Fee,
      0 as Barter_Rebate,
      ...
      sum(Exch_Cost) as Exch_Cost,
      sum(SSP_Platform_Fee) as SSP_Platform_Fee,
      sum(Pub_Platform_Fee) as Pub_Platform_Fee,
      sum(Barter_Rebate) as Barter_Rebate,
      sum(Partner_RevShare) as Partner_RevShare,

      case when buy_type = 'Self-Service' THEN sum(coalesce(DSP_Revenue,0)) + sum(coalesce(DSP_Platform_Cost,0)) + sum(coalesce(DSP_PlatformFee_Profit,0)) + sum(coalesce(SSP_Platform_Fee,0)) + sum(coalesce(Pub_Platform_Fee,0)) + sum(coalesce(Barter_Rebate,0)) + sum(coalesce(Partner_RevShare,0))
      ELSE sum(coalesce(DSP_Revenue,0)) + sum(coalesce(DSP_PlatformFee_Profit,0)) + sum(coalesce(SSP_Platform_Fee,0)) + sum(coalesce(Pub_Platform_Fee,0)) + sum(coalesce(Barter_Rebate,0)) + sum(coalesce(Partner_RevShare,0)) END AS Gross_Revenue,

      sum(coalesce(Exch_Cost,0)) + sum(coalesce(DSP_Data_Cost,0)) + sum(coalesce(DSP_Partner_Cost,0)) as Overall_Cost
      From E2E_BASE_Metrics
      Group by 1, 2, 3, 4, 5
      ;;
  }
