view: v_daily_core_stats_yesterday {

  derived_table: {
    sql:
    SELECT dcs.flight_media_id
      {% if v_placement_details_base._in_query %} ,dcs.placement_id {% endif %}
    ...
    {% if v_placement_details_base._in_query %} JOIN dim.placement_details_base_view pd on dcs.placement_id = pd.placement_id {% endif %}
...
WHERE 1=1
  ...
  dimension: primary_key {
    ...
    primary_key: yes
    hidden: yes
    sql: ${flight_media_id} {% if v_placement_details_base._in_query %} || ${placement_id} {% endif %} ;;
  }
