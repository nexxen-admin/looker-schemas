...
JOIN dim.DIM.FLIGHT_MEDIA_DETAILS_BASE_VIEW fmd ON (dds.FLIGHT_MEDIA_ID = fmd.FLIGHT_MEDIA_ID)
JOIN dim.DIM.CAMPAIGN_DETAILS_BASE_VIEW cd ON (fmd.campaign_id = cd.campaign_id)
JOIN dim.DIM.INSERTION_ORDER_DETAILS_VIEW io ON (cd.insertion_order_id = io.insertion_order_id)
WHERE (
  1=1
  AND {% condition v_campaign_details_base.campaign_id %} fmd.CAMPAIGN_ID  {% endcondition %}
  AND {% condition v_campaign_details_base.platform_client_id %} cd.PLATFORM_CLIENT_ID  {% endcondition %}
  AND {% condition v_campaign_details_base.platform_client_description %} cd.PLATFORM_CLIENT_DESCRIPTION  {% endcondition %}
  AND {% condition v_campaign_details_base.is_test_campaign %} cd.IS_TEST_CAMPAIGN  {% endcondition %}
  AND {% condition v_campaign_details_base.isfiller %} cd.ISFILLER  {% endcondition %}
  AND {% condition v_campaign_details_base.campaign_status_description %} cd.CAMPAIGN_STATUS_DESCRIPTION  {% endcondition %}
  AND {% condition v_campaign_details_base.business_region_description %} cd.BUSINESS_REGION_DESCRIPTION  {% endcondition %}
