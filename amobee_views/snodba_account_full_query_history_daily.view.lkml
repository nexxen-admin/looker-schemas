view: account_full_query_history_daily {
    derived_table: {
      sql:
         SELECT
                                           t1.WAREHOUSE_NAME,
                                           ...
                                           from (
                                                ...
                                                warehouse_name,
                                                DATE_TRUNC('day', end_time) CAPTURE_DAY,
                                                WAREHOUSE_SIZE
                                                from "SNOWFLAKE"."ACCOUNT_USAGE"."QUERY_HISTORY"
                                                where WAREHOUSE_SIZE is not null
                                                group by warehouse_name, CAPTURE_DAY, WAREHOUSE_SIZE) t1
                                            join (
                                              ...
                                              group by warehouse_name,CAPTURE_DAY,QUERY_TEXT,ROLE_NAME,USER_NAME,WAREHOUSE_SIZE
                                            ) t2
                                            ON t1.warehouse_name = t2.warehouse_name AND t1.CAPTURE_DAY = t2.CAPTURE_DAY
                                               AND t1.WAREHOUSE_SIZE=t2.WAREHOUSE_SIZE
                                            join(
                                              SELECT warehouse_name,
                                                 DATE_TRUNC('day', start_time) CAPTURE_DAY,
                                                 SUM(CREDITS_USED) as CREDITS_USED_by_day,
                                                 SUM(CREDITS_USED_COMPUTE) as CREDITS_USED_COMPUTE,
                                                 SUM(CREDITS_USED_CLOUD_SERVICES) as CREDITS_USED_CLOUD_SERVICES
                                              FROM ACCOUNT_USAGE.WAREHOUSE_METERING_HISTORY
                                              group by warehouse_name, CAPTURE_DAY
                                            ) t3
                                            ON t3.warehouse_name = t2.warehouse_name AND t3.CAPTURE_DAY = t2.CAPTURE_DAY
          ;;
    }
