view: hourly_analytics_derived {
  ...
  derived_table: {
    sql: select TO_DATE(hourly_analytics.DEMAND_DATE) as demand_date, COALESCE(SUM(hourly_analytics.IMPRESSIONS), 0) as sum_impressions, COALESCE(SUM(CASE WHEN hourly_analytics.ADSERVER_CODE = 1 then hourly_analytics.PUBLISHER_REQUESTS else 0 end ), 0) as sum_served_requests from RAWDB.analytics_view AS hourly_analytics where hourly_analytics.DEMAND_DATE >= DATEADD('day', -60, CURRENT_DATE()) group by TO_DATE(hourly_analytics.DEMAND_DATE);;
  }
  ...
  measure: impressions_change_relative_to_previous_day {
    ...
    sql: (${hourly_analytics.sum_impressions} - ${sum_impressions}) / nullif(${sum_impressions}, 0) ;;
  }
  ...
  measure: served_requests_change_relative_to_previous_day {
    ...
    value_format_name: percent_2
    sql: (${hourly_analytics.sum_served_requests} - ${sum_served_requests}) / nullif(${sum_served_requests}, 0) ;;
  }
