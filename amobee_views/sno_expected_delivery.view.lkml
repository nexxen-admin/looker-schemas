  ...
  derived_table: {
    sql:  WITH _FLIGHT_STATS AS (
            ...
            WHERE
              ...
              {% condition campaign_details_base.campaign_id %} fd.CAMPAIGN_ID {% endcondition %} AND
              {% condition flight_details.flight_id %} fd.FLIGHT_ID {% endcondition %}
            GROUP BY
              fd.CAMPAIGN_ID,
              fd.FLIGHT_ID
          )
