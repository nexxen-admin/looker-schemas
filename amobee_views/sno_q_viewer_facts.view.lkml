view: sno_q_viewer_facts {
  derived_table: {
    sql:
        SELECT
        {% if sno_q_viewer_facts.age._in_query %} v.age, {% endif %}
        {% if sno_q_viewer_facts.network._in_query %} mc.media_content_parent, {% endif %}
        {% if sno_q_viewer_facts.daypart._in_query %} mc.media_daypart_id , {% endif %}
        {% if sno_q_viewer_facts.program._in_query %} mc.media_content_name , {% endif %}
        vf.viewer_key
      ...
      JOIN VIEWERSHIP.MEDIA_CONTENT mc
       ...
       GROUP BY {% if sno_q_viewer_facts.age._in_query %}  v.age, {% endif %}
                {% if sno_q_viewer_facts.network._in_query %} mc.media_content_parent, {% endif %}
                {% if sno_q_viewer_facts.daypart._in_query %} mc.media_daypart_id , {% endif %}
                {% if sno_q_viewer_facts.program._in_query %} mc.media_content_name , {% endif %}
  ...
  filter: timeperiod_quarter_key {
    view_label: "Viewing Metrics"
    description: "Select the quarter to get ad reach/frequency report. Eg. 2017 Q1 is 201701"
    label: "Quarter"
    type: number
  }
