view: daily_core_stats_yesterday_demand {
  ...
  JOIN (
    ...
    WHERE schema_name = 'demand_mart'
      AND table_name = 'daily_core_stats_view'
...
GROUP BY
  ...
  measure: estimated_daily_ccp_yesterday {
    ...
    view_label: "Yesterday Impression Metrics"
    value_format_name: decimal_0
    sql: case when dateadd(d, 1, max(${demand_mart_load_tracking.load_through_date})) > coalesce(max(${flight_details.begin_datetm_local_raw}), max(${flight_details.contract_start_date})) and
      max(${demand_mart_load_tracking.load_through_date}) <= coalesce(max(${flight_details.end_datetm_local_raw}), max(${flight_details.contract_end_date})) then
      greatest(0, coalesce(${flight_demand_units_budget.sum_units}, 0) - coalesce(${daily_core_stats.sum_billable_units}, 0) + coalesce(${sum_billable_units_yesterday}, 0)) /
      nullif(least(${flight_details.max_flight_total_days}, ${flight_details.max_flight_remaining_days} + 1), 0)
      else 0 end ;;
  }
