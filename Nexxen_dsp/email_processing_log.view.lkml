    ...
    sql:
        CASE
            ...
            WHEN LOWER(SUBSTRING(CAST(${file_processing_error_reason} AS VARCHAR(10000)) FROM 1 FOR 10000)) LIKE '%missing tpa_external_id%' OR
                 LOWER(SUBSTRING(CAST(${file_processing_error_reason} AS VARCHAR(10000)) FROM 1 FOR 10000)) LIKE '%missing ''tpa_external_id'' or ''date'' in the file%' THEN 'Missing tpa_external_id or date'
