view: jbs_adaptable_4_13_2022 {
  ...
  derived_table: {
    sql: SELECT  date::date as "Date",
        ...
        CASE WHEN flight_id IN(4206406, 42222036, 4244826, 4267946, 4301536, 4339246, 4371406, 4396786, 4431676) THEN 'CTV + BT + DMA Targeting'
           WHEN flight_id IN(4206676, 4222046, 4244836, 4268006, 4301546, 4339256, 4371416, 4396796, 4431736) THEN 'CTV + BT + Zip Targeting'
           WHEN flight_id IN(4206726, 4222056, 4244846, 4268016, 4301556, 4339266, 4371426, 4396806,4431766) THEN 'All Screen Video OTT + BT + DMA Targeting'
           WHEN flight_id  IN(4206736, 4222066, 4244856, 4268026, 4301566, 4339296, 4371436, 4396816, 4431786) THEN ' ALL Screen Video OTT + BT + Zip Code Targeting'
        ...
        CASE WHEN flight_id IN (4206406,
                    ...
                    4431766,
                    4431736,
                    4431676) THEN (SUM(impressions)/1000) * 21.50
             ELSE 0 END AS "Spend"
